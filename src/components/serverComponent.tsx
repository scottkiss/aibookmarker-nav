...
export default function ServerSideComponent({ initialSites }: { initialSites: SiteProps["site"][] }) {
  ...
  return (
    <div className="flex flex-col lg:flex-row">
      <aside className="lg:w-64 p-4 bg-gray-100 shadow-lg lg:fixed lg:h-full overflow-y-auto">
        <h2 className="font-bold mb-4">Categories</h2>
        <nav aria-label="Category navigation">
          <ul className="flex flex-wrap lg:flex-col space-y-0 space-x-2 lg:space-x-0 lg:space-y-2">
            {Object.keys(categories).map((tag) => (
              <li key={tag}>
                <a href={`#${tag}`} className="text-lg block p-2 rounded-md text-gray-700 hover:bg-gray-200 transition-colors">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {categories[tag].map((site) => (
                <div key={site.id} className="bg-white p-4 shadow rounded-lg transition-transform transform hover:scale-105">
                  ...
                  <a href={site.url} className="mt-4 inline-block text-center bg-blue-500 text-white rounded-md px-4 py-2 hover:bg-blue-600 transition-colors">
                    Visit
                  </a>
                </div>
              ))}
